...
function parseInline(text, baseSize) {
    ...
    while ((match = regex.exec(text)) !== null) {
        if (match.index > lastIndex) {
            runs.push(new TextRun({ text: text.slice(lastIndex, match.index), size: baseSize, font: "Arial" }));
        }
        if (match[1] !== undefined) {
            runs.push(new TextRun({ text: match[1], bold: true, size: baseSize, font: "Arial" }));
        } else {
            runs.push(new ExternalHyperlink({
                link: match[3],
                children: [new TextRun({ text: match[2], size: baseSize, font: "Arial", style: "Hyperlink" })]
            }));
        }
        lastIndex = regex.lastIndex;
    }
    if (lastIndex < text.length) {
        runs.push(new TextRun({ text: text.slice(lastIndex), size: baseSize, font: "Arial" }));
    }
    return runs;
}
...
function sectionHeader(text) {
    return new Paragraph({
        ...
        children: [new TextRun({ text: text.toUpperCase(), bold: true, size: 19, color: NAVY, font: "Arial" })]
    });
}
...
function titleDateLine(title, date) {
    return new Paragraph({
        ...
        children: [
            ...
            new TextRun({ text: `\t${date}`, size: 19, color: GRAY, font: "Arial" })
        ]
    });
}
...
function buildHeader(fields) {
    ...
    fields.links.forEach((l, i) => {
        ...
        linkRuns.push(new ExternalHyperlink({
            link: l.url,
            children: [new TextRun({ text: l.label, size: 18, font: "Arial", style: "Hyperlink" })]
        }));
    });
    ...
    return [
        new Paragraph({
            spacing: { after: 20 },
            children: [new TextRun({ text: fields.name, bold: true, size: 32, color: NAVY, font: "Arial" })]
        }),
        new Paragraph({
            spacing: { after: 60 },
            children: [new TextRun({ text: fields.title, size: 21, color: GRAY, font: "Arial" })]
        }),
        new Paragraph({
            spacing: { after: 30 },
        ...
        if (block.type === "h2") {
            ...
            if (parts.length === 2) {
                ...
                children.push(new Paragraph({
                    spacing: { after: 10 },
                    children: [new TextRun({ text: block.text, bold: true, size: 20, font: "Arial" })]
                }));
            }
        } else if (block.type === "list") {
            const plainSections = ["Competências Técnicas", "Idiomas"];
            if (plainSections.includes(currentSection)) {
                block.items.forEach((item, i) => {
                    children.push(new Paragraph({
                        spacing: { after: i === block.items.length - 1 ? 0 : 16 },
            ...
            if (isTechLine(block.text)) {
                ...
                children.push(new Paragraph({
                    spacing: { after: 10 },
                    children: [new TextRun({ text: inner, italics: true, size: 18, color: GRAY, font: "Arial" })]
                }));
                ...
                while ((m = linkRegex.exec(block.text)) !== null) {
                    if (!first) linkRuns.push(new TextRun({ text: "  |  ", size: 18, font: "Arial" }));
                    linkRuns.push(new ExternalHyperlink({
                        link: m[2],
                        children: [new TextRun({ text: m[1], size: 18, font: "Arial", style: "Hyperlink" })]
                    }));
                    first = false;
                }
                children.push(new Paragraph({ spacing: { after: 100 }, children: linkRuns }));
            } else {
                children.push(new Paragraph({
                    spacing: { after: 0 },
...
function main() {
    ...
    const doc = new Document({
        ...
        sections: [{
            ...
            children: [...buildHeader(fields), ...buildBody(blocks)]
        }]
    });
    ...
    Packer.toBuffer(doc).then(buffer => {
        ...
        try {
            execSync(`soffice --headless --convert-to pdf "${docxPath}"`, { stdio: "inherit" });
        } catch (e) {
