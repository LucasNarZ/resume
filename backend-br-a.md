...
Plataforma full stack em produção. APIs REST, modelagem relacional, autenticação, monitoramento com Prometheus e Grafana, deploy automatizado na AWS com pipeline CI/CD.

- **Tecnologias**: NestJS, Sequelize, Next.js, PostgreSQL, Jest, AWS, GitHub Actions, Docker, Prometheus, Grafana
- Código: https://github.com/LucasNarZ/Sunlog.dev
- Live demo: https://sunlog.dev

### PCBuilderBR - Motor de compatibilidade e precificação para builds

Plataforma full stack com backend em FastAPI e PostgreSQL, responsável por centralizar catálogo, regras de compatibilidade e cálculo do preço total da configuração em tempo real.

- **Tecnologias**: FastAPI, Pydantic, PostgreSQL, Docker, React, Vite, TailwindCSS, react-hook-form, Zod, Axios
- O projeto cobre validações entre socket, chipset, DDR, conectores, TDP e limites físicos do gabinete, além de um worker diário para atualização de preços em Kabum, Pichau e Amazon.
...
- Índices (B-Tree/B+Tree), paginação, particionamento, transações ACID e funcionamento interno de SGBDs
